...
from user.forms import UserProfileClimbingForm
from user.models import Climbing, UserProfile


class UserProfileClimbingView(LoginRequiredMixin, UpdateView):
    template_name = 'user/main.html'
    form_class = UserProfileClimbingForm
    success_url = reverse_lazy('user:climbing')
    context_object_name = 'profile'

    def get_object(self, queryset=None):
        return Climbing.objects.get(profile=UserProfile(user=self.request.user))
